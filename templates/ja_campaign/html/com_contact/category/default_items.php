		...
		<fieldset class="com-contact-category__filters filters btn-toolbar">
			...
			<?php if ($this->params->get('show_pagination_limit')) : ?>
				<div class="com-contact-category__pagination btn-group float-right">
					<label for="limit" class="sr-only element-invisible">
				...
				<?php foreach ($this->items as $i => $item) : ?>
					...
					<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
						...
						<?php if ($this->params->get('show_image_heading')) : ?>
							...
							<div class="col-md-2">
								<?php if ($this->items[$i]->image) : ?>
									<?php 
										//t4 override
										$headingLink = (class_exists('ContactHelperRoute')) ? ContactHelperRoute::getContactRoute($item->slug, $item->catid, $item->language) : J4ContactHelperRoute::getContactRoute($item->slug, $item->catid, $item->language);
									?>
									<a href="<?php echo Route::_($headingLink); ?>">
						...
						<div class="list-title col-md-<?php echo $contact_width; ?>">
							<?php 
								//t4 override
								$contact_widthLink = (class_exists('ContactHelperRoute')) ? ContactHelperRoute::getContactRoute($item->slug, $item->catid, $item->language) : J4ContactHelperRoute::getContactRoute($item->slug, $item->catid, $item->language);
							?>
							<a href="<?php echo Route::_($contact_widthLink); ?>">
